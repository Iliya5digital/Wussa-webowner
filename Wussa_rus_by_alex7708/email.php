...
if (isset($_POST["emailImage"])) {
 ...
 if ($to == "") 
    echo "<script language=\"JavaScript\">alert('Введите e-mail!');</script>";
 else {
    $headers = "От: $webtitle <{$website}>\r\nContent-type: text/html"; 
    $body = "<HTML><BODY>
             <br><center><h1>$webtitle - Хостинг изображений!</h1><br><br>
             <a href=\"$path\"><img src=\"{$website}/thumb.php?id=$id\"></a>  
             <br><br><br><LABEL id='title'>Ссылка на изображение:</LABEL><br>$path
             <br><br>
    ...
    if (mail($to, "$webtitle - Image Link", $body, $headers)) 
       ...
       echo "<script language=\"JavaScript\">alert('Письмо не может быть отправлено!');</script>";
 }
} 
