...
if (isset($_POST['member'])) {
   $member = $_POST['member'];
   if (trim($member) == "") die("Введите логин пользователя!");
   ...
   if ($n) {
     ...
     echo "<td> <b><u>Логин</u></b></td> 
           <td> <b><u>Пароль</u></b></td>
           <td> <b><u>Пользовательский пароль</u></b></td>
           <td> <b><u>Email</u></b></td>
           <td> <b><u>Действие</u></b></td></tr>";
       ...
       echo "<td>{$r['username']}</td> 
        ...
        <td> {$r['userpass']}</td>
        <td> {$r['email']}</td>
        <td>  <a href=\"members.php?delete={$r['id']}\">Удалить</a></b></td>";
       echo "</tr></table>";
       echo "<center><br><br><a href='members.php'>Назад</a></center>";
       die();
   }   
   else
...
if (isset($_GET["delete"])) {
  ...
  if(mysql_query("DELETE FROM `members` WHERE id = '$id'"))
      echo "<b>Пользователь удалён!</b>";
  else
      echo "<b>Пользователь не может быть удалён!</b>";
...
echo "Введите логин пользователя: <input type=text name='member' size=40>";
echo "&nbsp; &nbsp;<input type=submit value='Посмотреть детали'></form></font></center><br>";
...
if ($number) {
  ...
  for ($i=$from1; $i < $from2; $i++) {
       ...
       echo "<td>{$musername[$i]}</td>
        ...
        <td> {$muserpass[$i]}</td>
        <td> {$memail[$i]}</td>
        <td>  <a href=\"members.php?delete={$mid[$i]}\">Удалить</a></b></td>";
       echo "</tr>";        
  ...
  {
     $previous = $page - 1;
     echo "<a href='members.php?page=$previous'><< Предыдущая</a>";
  } echo "</td>";
  ...
  {
     $next = $page + 1;
     echo "<a href='members.php?page=$next'>Следующая >></a>";
  } echo "</td></tr></table>";
  ...
  echo "Зарегистрированных пользователей нет!";
