...
if (isset($_POST["tags1"])) {
   ...
   if ($number) die("Ваш IP заблокирован. <br><br><a href='index.php'>Вернуться на главную</a>");
           ...
           if ( (trim($_POST["tags" . $i]) != "") ) {
                ...
                $size = intval(($_FILES[$name]["size"] / 1024) / 1024);
                 ...
                 if ($session == true) 
                   ...
                   if ($size > $limit)
                       die ("Размер изображения превышает $limit Mb лимит.");
                   ...
                   die("\"{$_FILES[$name]["name"]}\" неправильное изображение.");
                ...
                { 
                 ...
                 echo "Изображение \"{$_FILES["image" . $i]["name"]}\" загружено. <br><br>";
                 

                 echo "<LABEL id='title'>HTML:</LABEL><br><input type='text' size=92 onclick=\"this.select();\" value=\"<a href='{$website}/show-image.php?id=$id'> <img src='{$website}/{$thumbPath}' alt='Хостинг изображений' border='0'> </a>\">";
                 echo "<br><br>";
    
 
                 echo "<LABEL id='title'>BB код:</LABEL><br><input type='text' size=92 onclick=\"this.select();\" value=\"[URL={$website}/show-image.php?id={$id}] [IMG]{$website}/{$thumbPath}[/IMG][/URL]\">";
                 echo "<br><br>";


                 echo "<LABEL id='title'>Прямая ссылка (HTML):</LABEL><br><input type='text' size=92 onclick=\"this.select();\" value=\"<a href='{$website}'> <img src='{$website}/{$imagePath}'> </a>\">";
                 echo "<br><br>";


                 echo "<LABEL id='title'>Прямая ссылка (BB код):</LABEL><br><input type='text' size=92 onclick=\"this.select();\" value=\"[URL={$website}] [IMG]{$website}/{$imagePath}[/IMG][/URL]\">";
                 echo "<br><br>";
                 ...
                 echo "<br><br><hr color='#233c9b'><br>";                      
                    ...
                    echo "Изображение \"{$_FILES["image" . $i]["name"]}\" слишком маленькое или слишком большое!<br><hr color='#b1ddf6'>";   

             }
             else
                    echo "Введите таг изображения \"{$_FILES["image" . $i]["name"]}\" <br><hr color='#b1ddf6'>";  
       }
   }
