...
function deleteGallery(id) { 
  x = confirm("Вы уверены что хотите удалить эту галерею ?"); 
  if (x) 
...
{    
    ...
    $show = true;    
            ...
            if ($name != "") {
                  if ($id != "") {
                      if(mysql_query("UPDATE `galleries` SET name='$name' WHERE (id='$id') AND (userid='$userid')")) {
                          echo "Название галереи изменено !";
                          echo "<br><br><a href=\"mygalleries.php\">Нажмите здесь</a> чтобы вернуться";
                      }
                      else
                          echo "Название галереи не может быть изменено!";
                  }
            }
            else {
                  echo "Введите название галереи !"; 
                  echo "<br><br><a href=\"mygalleries.php\">Нажмите здесь</a> чтобы вернуться";
            }
       }
          ...
          if ($tags != "") {
              if ($id != "") {
                ...
                if(mysql_query("UPDATE `images` SET tags='$tags' WHERE id='$id'")) {
                   echo "Таги изображений были изменены !";
                   echo "<br><br><a href=\"mygalleries.php\">Нажмите здесь</a> чтобы вернуться";
                }
                else
                   echo "Таги изображений не могут быть изменены!";
             }
          }
          else {
              echo "Введите таг!"; 
       ...
       if (isset($_POST["deleteselected"])) {
          ...
          if (isset($_POST["images"])) {
               ...
               echo "<br><br><a href=\"mygalleries.php?act=show&id=$galleryid\">Нажмите здесь</a> чтобы вернуться";
          }
          else {
   ...
   if (isset($_POST["moveselectedimages"])) {
     ...
     if (isset($_POST["images"])) {
        ...
        echo "Выберите изображение для перемещения!";
        echo "<br><br><a href=\"mygalleries.php?act=show&id=$galleryid\">Нажмите здесь</a> чтобы вернуться";
     }
            ...
            if ($_GET['name'] != "") {
               if (($_GET['type'] == "private") OR ($_GET['type'] == "public")) {
                 ...
                 if (mysql_query("INSERT INTO `galleries` (id, userid, name, type) VALUES('$id', '$userid', '$name', '$type')"))
                    echo "Галерея создана !";
                 else
                    echo "Галерея не может быть создана!";
               }
               else echo "Неправильная категория.";
                  
            }
            else
                echo "Введите название галереи!";

            echo "<br><br>Вернуться в <a href='mygalleries.php'>'Мои галереи'</a>";
      ...
      if (isset($_GET["act"])) {
         ...
         if (isset($_GET["id"]))
          ...
          if ($act == "deletegallery") {
             ...
             $n = mysql_num_rows($result);
             if (!$n) die("Галерея не может быть удалена!");
             
             if (mysql_query("DELETE FROM `galleries` WHERE (id = '$id') AND (userid = '$userid')"))
               echo "Галерея удалена!";
             else
               echo "Галерея не может быть удалена!";
          ...
          if ($act == "show") {
             ...
             if ($n) {
                ...
                echo "<center>";
                echo "<br><h1>Галерея '$name' изображений ($type)</h1><br>";
                echo "<b>Ссылка на галерею:</b> {$website}/gallery.php?id=$galleryid <br>";
                echo "В этой галереи '$n' изображений<br><br><br>";
                ...
                echo "&nbsp;&nbsp; <input type='submit' name='moveselectedimages' value=\"Переместить в 'Мои изображения'\">";
                ...
                while ($row = mysql_fetch_array($result_set))
                {
                  ...
                  echo "<td align=center> <a href=\"mygalleries.php?id=$id&act=changetags\">Изменить таги</a>
                        &nbsp; | &nbsp; <a href=\"mygalleries.php?act=deleteimage&id=$id\">Удалить</a>
                        </td>";
                ...
                echo "<center>Ни одного изображения не было загружено в эту галерею!</center>";
         ...
         if ($act == "deleteimage") {
          ...
          if ($act == "makeprivate") {
              ...
              if(mysql_query("UPDATE `galleries` SET type='private' WHERE (id='$id') AND (userid='$userid')")) {
                  echo "Личная галерея установлена!";
                  echo "<br><br><a href=\"mygalleries.php\">Нажмите здесь</a> чтобы вернуться";
              }
              else
                  echo "Галерея не может быть личной!";
          }
          ...
          if ($act == "makepublic") {
             ...
             if(mysql_query("UPDATE `galleries` SET type='public' WHERE (id='$id') AND (userid='$userid')")) {
                  echo "Общая галерея установлена!";
                  echo "<br><br><a href=\"mygalleries.php\">Нажмите здесь</a> чтобы вернуться";
             }
             else
                  echo "Галерея не может быть общей!";
          }
          ...
          if ($act == "changename") {
               ...
               echo "<form method='POST' action='mygalleries.php'>";
               echo "<br>Введите новое название галереи:<br><input type='text' name='name' size=40 maxlength=250>";
          ...
          echo "<br><br><center><a href='mygalleries.php'>Вернуться в Мои галереи</a></center>";    
         ...
         if ($show == true) {
            ...
            echo "<br><center><h1>Мои галереи</center></h1><br>"; 
            ...
            if ($n) {
                 ...
                 while ($row = mysql_fetch_array($result_set))
                 {
                       ...
                       echo "<br><br> <a href=\"mygalleries.php?act=show&id={$row['id']}\">Управление</a> 
                              | <a href=\"mygalleries.php?act=changename&id={$row['id']}\">Переименовать</a>
                              | <a href='#' onclick=\"deleteGallery('{$row['id']}');\">Удалить</a> 
                              <br>";
                     
                       if ($row['type'] == "public")  
                          echo "<a href=\"mygalleries.php?id={$row['id']}&act=makeprivate\">Сделать личной</a>";
                       else
                          echo "<a href=\"mygalleries.php?id={$row['id']}&act=makepublic\">Сделать общей</a>";
                      
                       echo "</td>";
                  }
                         
                  echo "</table></center><br><hr color=#b1ddf6>";
                  echo "<center><br><br><b>Примечание:</b>Нажмите на 'Управление' для изменения изображений !<br><br>";
           }
           else
                  echo "<center><b>Не было создано ни одной галереи!</b><center><br>";


           echo "<br><br><h2>Создать галерею</h2><br>";
           echo "<form action='mygalleries.php' method='GET' name='myForm1'>"; 
           echo "<table><tr><td width=100><LABEL id='message'>Название: </LABEL></td> <td><input type=text name='name' maxlength=250></td></tr>";
           echo "<tr><td><LABEL id='message'>Тип: </LABEL></td>
                 <td>
                    <select name=type>
