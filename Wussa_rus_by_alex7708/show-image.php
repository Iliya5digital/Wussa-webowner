...
$id = $_GET["id"];
if ($id == "") die("Введи ID изображения!");

$result = mysql_query("SELECT * FROM `images` WHERE id='$id'");
$number = mysql_num_rows($result);
$row = mysql_fetch_array($result);
if (!$number) die("Изображения не существует,либо оно удалено из-за несоответствия нашим <a href='terms.php'>Правилам</a> !");
...
if ($type == "member-private") {
  ...
  if ($row['userid'] != $userid) {
      echo "Это личное изображение.Введите пароль,чтобы посмотреть его<br><br>";
      echo "<form action='show-image.php' method='POST'>
           <LABEL id='title'>Пароль:</LABEL> <input type='password' name='password' maxlength='30'>";
    ...
    if ($imguserid == $userid) {
       show();
         ...
         if (isset($_SESSION['gallery' . $galleryid]))
              ...
              echo "Это личное изображение.Введите пароль,чтобы посмотреть его<br><br>";
              echo "<form action='show-image.php' method='POST'>Пароль: <input type='password' name='password' maxlength='30'>";
...
function show() {
  ...
  echo "<center>";
  echo "<br><LABEL id='title'>Просмотров:</LABEL> $views";
  echo "<br><LABEL id='title'>Дата добавления:</LABEL> {$row['date']}";
  ...
  //***********************************************************************************
     ...
     { 
          ...
          echo "<br><br><a href='$image' title='Нажмите здесь,чтобы увидеть изображение' target='_blank'>
  ...
  echo "<br><br><LABEL id='title'>Детали:</LABEL> {$row['details']}";
  echo "<br><LABEL id='title'>Загружено:</LABEL> $username";
  echo "<br><br><LABEL id='title'>Таги:</LABEL><br>{$row['tags']}";
  
  echo "<br><br><br><br>";

  echo "<div class='emailBox'>
        <form method='POST' action='email.php'>
        <LABEL id='title'>Отправить изображение другу:</LABEL><br> &nbsp; <input type='text' size='20' name='email'>&nbsp; &nbsp;
        <input type='submit' value='Отправить' name='emailImage'>
...
if ($imguserid != -1) {  
     ...
     while ($row1 = mysql_fetch_array($r)) {
       ...
       if ($ccount == 5) 
         echo "<br><a href='#' style='FONT-SIZE: 16px'
                 ...
                 Посмотреть все комментарии</a>
               <div style='display: none' id='allcomments'>";

       echo "<div class='commentbox'> $comment ";
       if ($own == true) 
           echo "<br><a href='deletecomment.php?id={$row1['id']}'>Удалить</a>";
       echo "</div>";
     }
      ...
      echo "<div class='commentbox'>Комментарии отсутсвуют!</div>";

    
  if ($session == true) {
 
    echo "<form method='POST' action='postcomment.php'>
          <br><h2>Написать комментарий</h2>
          <textarea cols=40 rows=4 name='comment'></textarea><br>
          <input type='hidden' name='id' value='$id'>
          <input type='submit' value='Написать комментарий' name='postcomment'>
  ...
  echo "<LABEL id='title'>HTML:</LABEL><br><input type='text' size=60 onclick=\"this.select();\" value=\"<a href='{$website}/show-image.php?id=$id'> <img src='{$website}/{$thumb}' alt='Хостинг изображений' border='0'> </a>\">";
  echo "<br><br>";
    
 
  echo "<LABEL id='title'>BB код:</LABEL><br><input type='text' size=60 onclick=\"this.select();\" value=\"[URL={$website}/show-image.php?id={$id}] [IMG]{$website}/{$thumb}[/IMG][/URL]\">";
  echo "<br><br>";


  echo "<LABEL id='title'>Прямая ссылка (HTML):</LABEL><br><input type='text' size=60 onclick=\"this.select();\" value=\"<a href='{$website}'> <img src='{$website}/{$image}'> </a>\">";
  echo "<br><br>";


  echo "<LABEL id='title'>Прямая ссылка (BB код):</LABEL><br><input type='text' size=60 onclick=\"this.select();\" value=\"[URL={$website}] [IMG]{$website}/{$image}[/IMG][/URL]\">";
  echo "<br><br>";
